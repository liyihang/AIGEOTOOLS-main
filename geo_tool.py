        ...
        if hasattr(st, 'secrets') and st.secrets:
            ...
            if "api_keys" in st.secrets:
                ...
                if "deepseek" in api_keys and api_keys["deepseek"]:
                    base_cfg["gen_api_key"] = _clean_api_key(api_keys["deepseek"])
                    base_cfg["verify_keys"]["DeepSeek"] = _clean_api_key(api_keys["deepseek"])
                if "tongyi_wanxiang" in api_keys and api_keys["tongyi_wanxiang"]:
                    base_cfg["tongyi_wanxiang_api_key"] = _clean_api_key(api_keys["tongyi_wanxiang"])
...
def validate_cfg(cfg: dict):
    ...
    gen_key = _clean_api_key(cfg.get("gen_api_key", ""))
    if not gen_key:
        errors.append("生成&优化 LLM 的 API Key 未填写")
    elif _has_non_ascii(gen_key):
        errors.append("生成&优化 LLM 的 API Key 包含非 ASCII 字符（可能是中文字符或特殊空格），请重新粘贴")
    ...
    for vp in verify_providers:
        vk = _clean_api_key(verify_keys.get(vp, ""))
        if not vk:
            errors.append(f"验证模型 {vp} 的 API Key 未填写")
        elif _has_non_ascii(vk):
            errors.append(f"验证模型 {vp} 的 API Key 包含非 ASCII 字符（可能是中文字符或特殊空格），请重新粘贴")
...
with st.sidebar:
    ...
    if apply_cfg or not st.session_state.cfg_applied:
        ...
        st.session_state.cfg = {
            ...
            "gen_api_key": _clean_api_key(gen_api_key),
            "verify_providers": verify_providers,
            "verify_keys": {vp: _clean_api_key(v) for vp, v in verify_keys.items()},
            "tongyi_wanxiang_api_key": _clean_api_key(tongyi_wanxiang_api_key),
            "brand": brand_value,
